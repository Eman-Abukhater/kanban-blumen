...
router.post("/image", upload_1.upload.single("image"), async (req, res) => {
    try {
        ...
        if (isProduction) {
            ...
            const uploadResult = cloudinary_1.default.uploader.upload_stream({
                folder: "kanban-cards",
                transformation: [
...
router.post("/multiple", auth_1.authenticateToken, upload_1.upload.array("files", 10), async (req, res) => {
    try {
