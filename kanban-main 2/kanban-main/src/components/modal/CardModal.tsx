...
export function CardModal(props: CardModalProps) {
  ...
  const handleSave = () => {
    ...
    // Use cloudinaryUrl if available (new upload), otherwise keep existing imageUrl
    ...
    console.log("💾 Saving card with data:", {
      ...cardData,
      imageUrl: finalImageUrl,
      isNewCloudinaryUpload: !!cloudinaryUrl,
      originalImageUrl: props.card.imageUrl,
    });
  ...
  const handleSubmitTask = async (kanbanTaskId: number, index: number) => {
    ...
    try {
      ...
      // Upload file to Cloudinary first if a file is selected
      if (taskFile) {
        try {
      ...
      // Submit task with Cloudinary URL
                    ...
                    <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
                      ...
                      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:gap-3">
                        ...
                        <div className="flex w-full flex-wrap gap-2">
                          ...
                          {isCreatingTag && (
                            <div className="animate-pulse rounded-lg bg-slate-300 px-3 py-1 dark:bg-slate-600">
                              <div className="h-5 w-16 rounded bg-slate-400 dark:bg-slate-500"></div>
                            </div>
                          )}
                          {kanbanTags.length < 6 && (
                            <button
                                        ...
                                        <form>
                                          ...
                                          <div style={{ paddingTop: "18px" }}>
                                            <button
                                              ...
                                              disabled={submitTask}
                                              type="submit"
                                              className=" font-small inline-flex items-center justify-center gap-1 rounded-md border border-transparent bg-emerald-700 px-3 py-1 text-base text-white transition-colors duration-150 disabled:cursor-not-allowed disabled:opacity-50 hover:bg-emerald-600"
                                            >
                                              {submitTask ? (
                                                <>
                                                  <svg
